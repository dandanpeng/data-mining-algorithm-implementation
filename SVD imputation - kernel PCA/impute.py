...
def svd_imputation_optimised(X=None,ranks=None,
    ...
    nonnan_indice=sp.where(sp.isnan(X)==False)
    #2. Use "test_size" % of the non-missing entries as test data
    index=random.sample(range(len(nonnan_indice[0])),int(test_size*len(nonnan_indice[0])))
    #3. Create a new training data matrix: Set all test-entries in training data to nan
    train=X.copy()
    train[nonnan_indice[0][index],nonnan_indice[1][index]]=sp.nan
    ...
    for rank in ranks:
        ...
        X_imputed=svd_imputation(train,rank)
        #4.2 Compute the mean squared error of imputed test data with original test data and store error in array
        error=mean_squared_error(X[nonnan_indice[0][index],nonnan_indice[1][index]],X_imputed[nonnan_indice[0][index],nonnan_indice[1][index]])
    ...
    optimal_rank=int(testing_errors.index((min(testing_errors)))+1)
