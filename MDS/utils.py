...
'''
Gobal Colors and Markers For Plotting
'''
plot_color = ['#F7977A','#FDC68A','#A2D39C','#6ECFF6','#8493CA','#BC8DBF','#F6989D','#FFF79A']
plot_markers = ['.','x','o','h','d','v','s','+','*','>']

'''
Example distance matrix with different distances between cities (in km)
'''
city_dist = sp.array([[0,505,697,243,305,356,685],
                      [505,0,1184,670,691,524,878],
                      [697,1184,0,685,739,765,1107],
                      [243,670,685,0,75,593,488],
                      [305,691,739,75,0,659,414],
                      [356,524,765,593,659,0,1035],
                      [685,878,1107,488,414,1035,0]])
'''
City names 
'''
city_names = sp.array(['Munich','Berlin','Rom','Zuerich','Basel','Vienna','Paris'])
...
'''
Generate image with missing values
Input: X: Orginal Data matrix of size n x m, where n is the number of samples and m the features
       per: percentage of missing values in data matrix
       seed: seed for random methods
Output: matrix with missing values
'''
def randomMissingValues(X=None,per=0.2,seed=0):
    #set seed
    sp.random.seed(seed)
    nelem = int(sp.floor(X.shape[0]*X.shape[1]*per))
    ind = sp.where(X>=0.0)
    ind_r = sp.random.choice(ind[0].shape[0],nelem,replace=False)
    D_missing = sp.array(X.copy(),dtype="float")
    D_missing[ind[0][ind_r],ind[1][ind_r]] = sp.nan
    return D_missing
